        ...
        <div className="flex items-center justify-between gap-1 lg:gap-3 xl:gap-5 h-14 sm:h-16 lg:h-16 2xl:h-20 flex-nowrap">
                ...
                >
                  <button
                    ...
                    className={`px-2 xl:px-3 py-1.5 rounded-xl text-xs xl:text-sm font-semibold transition-all flex items-center gap-1.5 whitespace-nowrap shrink-0 ${
                      isActive 
                        ? 'text-[#E31F1F] dark:text-[#FF3333] bg-[#E31F1F]/10 dark:bg-[#FF3333]/15 font-bold' 
                        : 'text-slate-700 dark:text-slate-200 hover:text-[#E31F1F] dark:hover:text-[#FF3333] hover:bg-surface-subtle'
                    }`}
                  >
                    {ItemIcon && <ItemIcon className="w-3.5 h-3.5 shrink-0" />}
                    <span className="whitespace-nowrap">{item.label}</span>
                    {hasSubmenu && (
                      <ChevronDown className={`w-3.5 h-3.5 shrink-0 transition-transform duration-200 ${activeMegaMenu === item.id ? 'rotate-180 text-[#E31F1F] dark:text-[#FF3333]' : 'text-[#7D797A]'}`} />
                    )}
                  </button>
                          ...
                          <button
                            ...
                            className={`w-full p-2.5 rounded-xl hover:bg-surface-subtle transition-all text-left rtl:text-right flex flex-col border ${
                              currentRoute === child.route 
                                ? 'bg-[#E31F1F]/10 text-[#E31F1F] dark:text-[#FF3333] border-[#E31F1F]/20' 
                                : 'border-transparent text-slate-800 dark:text-slate-200'
                            }`}
                          >
                            <span className="text-xs font-bold">{child.label}</span>
                            <span className="text-[11px] text-[#7D797A] line-clamp-1">{child.desc}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                      ...
                      <div className="bg-surface-raised rounded-2xl border border-surface-border shadow-2xl p-6 glass-panel grid grid-cols-12 gap-6">
                        {item.id === 'solutions' && (
                          <>
                            <div className="col-span-8 space-y-3">
                              <div className="text-xs font-mono uppercase tracking-wider text-[#E31F1F] dark:text-[#FF3333] font-bold">
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                {(solutions || []).slice(0, 4).map((sol) => (
                                  ...
                                  >
                                    <div className="text-xs font-bold text-black dark:text-white dark:text-white group-hover/card:text-[#E31F1F] dark:group-hover/card:text-[#FF3333] transition-colors">
                                      {sol.name}
                                    </div>
                                    <div className="text-[11px] text-[#7D797A] line-clamp-1 mt-0.5">
                            ...
                            <div className="col-span-4 bg-surface-subtle p-4 rounded-xl border border-surface-border flex flex-col justify-between">
                              <div>
                                <span className="text-[10px] font-mono font-bold text-[#E31F1F] dark:text-[#FF3333] uppercase">{t.solutionsSec?.megaTitle || 'Enterprise Assurance'}</span>
                                ...
                                <p className="text-[11px] text-[#7D797A] mt-1">
                              ...
                              <button
                                ...
                                className="mt-3 text-xs font-bold text-[#E31F1F] dark:text-[#FF3333] hover:underline flex items-center gap-1 whitespace-nowrap"
                              >
                        ...
                        {item.id === 'industries' && (
                          <div className="col-span-12">
                            <div className="text-xs font-mono uppercase tracking-wider text-[#E31F1F] dark:text-[#FF3333] font-bold mb-3">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                              {(industries || []).map((ind) => (
                                ...
                                >
                                  <div className="text-xs font-bold text-black dark:text-white dark:text-white group-hover/ind:text-[#E31F1F] dark:group-hover/ind:text-[#FF3333]">
                                    {ind.name}
                                  </div>
                                  <div className="text-[11px] text-[#7D797A] mt-0.5 line-clamp-1">
                                    {ind.heroTag}
                                  </div>
                                </button>
                              ))}
                        ...
                        {item.id === 'global-compliance' && (
                          <div className="col-span-12 flex items-center gap-6">
                            <div className="flex-1 space-y-4">
                              ...
                              <p className="text-sm text-slate-600 dark:text-[#7D797A] leading-relaxed">
                              ...
                              <button onClick={() => onNavigate('global-compliance')} className="mt-4 px-4 py-2 rounded-lg bg-[#E31F1F] text-white text-xs font-bold hover:bg-[#2A2E35] transition-colors">
          ...
          <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
                  ...
                  {(supportedLanguages || []).map(lang => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2 text-sm transition-colors flex items-center justify-between ${
                        language === lang.code 
                          ? 'bg-[#E31F1F]/10 dark:bg-[#FF3333]/15 text-[#E31F1F] dark:text-[#FF3333] font-bold' 
            ...
            <button
              ...
              className="hidden md:inline-flex items-center gap-1.5 px-3.5 xl:px-4 py-2 rounded-xl bg-[#E31F1F] hover:bg-[#B42318] text-white font-bold text-xs shadow-md hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all whitespace-nowrap shrink-0 ml-1 cursor-pointer"
            >
        ...
        <div className="lg:hidden bg-surface-raised border-b border-surface-border px-4 py-6 space-y-4 animate-fade-in">
            ...
            {navItems.map((item) => (
              item.children ? (
                <div key={item.id} className="space-y-1 pl-1">
                  {item.children.map(child => (
                    <button
                      ...
                      className={`w-full p-2.5 rounded-xl text-left rtl:text-right text-sm font-semibold flex items-center justify-between whitespace-nowrap ${currentRoute === child.route ? 'text-[#E31F1F] dark:text-[#FF3333] bg-[#E31F1F]/10 font-bold' : 'text-slate-800 dark:text-slate-200'}`}
                    >
                      <span>{child.label}</span>
                      <ArrowRight className="w-4 h-4 text-[#7D797A] rtl:rotate-180" />
                    </button>
                  ))}
                </div>
              ) : (
                <button
                  ...
                  className={`w-full p-2.5 rounded-xl text-left rtl:text-right text-sm font-semibold flex items-center justify-between whitespace-nowrap ${currentRoute === item.route ? 'text-[#E31F1F] dark:text-[#FF3333] bg-[#E31F1F]/10 font-bold' : 'text-slate-800 dark:text-slate-200'}`}
                >
          ...
          <div className="pt-4 border-t border-surface-border flex flex-col gap-2">
            <button
              ...
              className="w-full py-3 rounded-xl bg-gradient-to-r from-[#E31F1F] to-[#E31F1F] dark:from-[#E31F1F] dark:to-[#FF3333] text-white dark:text-[#030303] font-bold text-sm shadow-md text-center whitespace-nowrap"
            >
