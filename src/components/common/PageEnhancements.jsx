    ...
    <div className="w-full overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-12 gap-10 items-center">
            <div className="lg:col-span-7">
              <span className="text-[11px] font-mono font-bold uppercase tracking-[0.2em] text-[#DD2A40] dark:text-[#FF3333]">
                ...
                {(e.steps || []).map((step, i) => (
                  <div key={i} className="p-4 rounded-2xl bg-surface-raised border border-surface-border hover:-translate-y-1 transition-transform">
                    <div className="flex items-center gap-2">
                      <span className="w-7 h-7 rounded-lg bg-[#DD2A40]/10 text-[#DD2A40] dark:text-[#FF3333] flex items-center justify-center text-[10px] font-bold">{String(i + 1).padStart(2, '0')}</span>
                      <span className="font-bold text-sm text-black dark:text-white dark:text-white">{step.title}</span>
                    </div>
                    <p className="mt-2 text-xs leading-5 text-[#667085] dark:text-[#667085]">{step.desc}</p>
                  </div>
                ))}
              </div>
            </div>
            <div className="lg:col-span-5">
              <div className="relative min-h-[340px] rounded-3xl eagle-pulse overflow-hidden border border-surface-border bg-[#DD2A40] shadow-2xl">
                ...
                <div className="absolute inset-0 bg-gradient-to-tr from-[#DD2A40] via-[#DD2A40]/70 to-transparent" />
      ...
      <section className="py-16 lg:py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto">
          <span className="text-[11px] font-mono font-bold uppercase tracking-[0.2em] text-[#DD2A40] dark:text-[#FF3333]">{e.servicesTag || 'SERVICE ARCHITECTURE'}</span>
          ...
          <p className="mt-3 text-sm text-[#667085] dark:text-[#667085]">{e.servicesSubtitle || 'Select a practice area to inspect its scope, typical deliverables and business value.'}</p>
        </div>
            ...
            {services.map((service, i) => {
              ...
              return (
                <button key={service.id || i} onClick={() => setActiveService(i)}
                  className={`w-full text-left rtl:text-right p-4 rounded-2xl border transition-all flex items-center gap-3 ${activeService === i ? 'bg-[#DD2A40] text-white border-[#DD2A40] shadow-lg' : 'bg-surface-raised border-surface-border hover:border-[#DD2A40]/50'}`}>
                  <span className={`w-10 h-10 rounded-xl flex items-center justify-center ${activeService === i ? 'bg-white/15' : 'bg-[#DD2A40]/10 text-[#DD2A40] dark:text-[#FF3333]'}`}><Icon className="w-5 h-5" /></span>
                  <span className="flex-1">
                    <span className="block font-bold text-sm">{service.name}</span>
                    <span className={`block mt-0.5 text-[11px] ${activeService === i ? 'text-white/75' : 'text-[#667085]'}`}>{service.category || service.shortDesc}</span>
                  </span>
                  <ArrowRight className={`w-4 h-4 ${activeService === i ? 'text-white' : 'text-[#667085]'} rtl:rotate-180`} />
                </button>
              );
            })}
          </div>
            ...
            <div className="lg:col-span-7 rounded-3xl overflow-hidden border border-surface-border bg-surface-raised shadow-xl">
              <div className="grid md:grid-cols-2 min-h-[330px]">
                ...
                <div className="p-7 flex flex-col justify-center">
                  <span className="text-[10px] font-mono uppercase tracking-wider text-[#DD2A40] dark:text-[#FF3333]">{e.selected || 'SELECTED PRACTICE'}</span>
                  ...
                  <div className="mt-5 space-y-2">
                    {(services[activeService].deliverables || ['Policies & procedures', 'Risk assessment framework', 'Testing and assurance', 'Management reporting']).slice(0, 4).map((x, i) => (
                      <div key={i} className="flex items-start gap-2 text-xs text-slate-600 dark:text-slate-300">
                        <CheckCircle2 className="w-4 h-4 text-[#DD2A40] dark:text-[#FF3333] shrink-0" /> {x}
                      </div>
                    ))}
                  </div>
                  <button onClick={() => onNavigate?.('solution-detail', { id: services[activeService].id })} className="mt-6 inline-flex items-center gap-2 text-xs font-bold text-[#DD2A40] dark:text-[#FF3333]">
      ...
      <section className="py-16 lg:py-24 bg-surface-subtle border-y border-surface-border">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <span className="text-[11px] font-mono font-bold uppercase tracking-[0.2em] text-[#DD2A40] dark:text-[#FF3333]">{e.faqTag || 'COMPLIANCE GUIDANCE'}</span>
          ...
          <div className="mt-10 space-y-3">
            {faqs.map(([q, a], i) => (
              <div key={i} className="rounded-2xl border border-surface-border bg-surface-raised overflow-hidden">
                <button onClick={() => setOpenFaq(openFaq === i ? -1 : i)} className="w-full p-5 text-left rtl:text-right flex items-center justify-between gap-4">
                  ...
                  <ChevronDown className={`w-4 h-4 text-[#DD2A40] transition-transform ${openFaq === i ? 'rotate-180' : ''}`} />
                </button>
            ...
            <div className="pt-2 flex justify-end">
              <button
                ...
                className="inline-flex items-center gap-1.5 text-xs font-bold text-[#DD2A40] dark:text-[#FF3333] hover:underline px-3 py-1.5 rounded-xl hover:bg-[#DD2A40]/10 dark:hover:bg-[#FF3333]/15 transition-all"
              >
          ...
          <div className="mt-10 p-6 rounded-3xl bg-[#DD2A40] text-white flex flex-col md:flex-row items-center justify-between gap-5">
            <div>
              <div className="text-xs font-mono uppercase tracking-wider text-[#E4E7EC]">{e.ctaTag || 'NEXT STEP'}</div>
            ...
            <button onClick={onOpenConsultation} className="px-5 py-3 rounded-xl bg-white text-[#DD2A40] font-bold text-xs hover:scale-[1.02] transition-transform">
