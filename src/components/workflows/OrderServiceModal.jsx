      ...
      >
        <div className="shrink-0 p-4 sm:p-5 bg-surface-subtle border-b border-surface-border flex items-center justify-between z-10">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-[#DD2A40]/10 dark:bg-[#FF3333]/15 border border-[#DD2A40]/20 dark:border-[#FF3333]/30 flex items-center justify-center text-[#DD2A40] dark:text-[#FF3333] shrink-0">
          ...
          <button 
            onClick={onClose} 
            className="p-2 rounded-xl bg-surface-base hover:bg-surface-raised text-[#667085] hover:text-black dark:text-white dark:hover:text-white border border-surface-border transition-colors cursor-pointer shrink-0 ml-2"
            title="Close modal"
            aria-label="Close"
          >
            <X className="w-5 h-5" />
          ...
          {isSuccess ? (
            <div className="text-center py-6 space-y-4 animate-scale-in">
              ...
              <p className="text-xs text-[#667085] max-w-md mx-auto">
                {m.orderSuccessDesc || 'Your request has been transmitted directly to EagleComply Counsel at'} <strong className="text-[#DD2A40] dark:text-[#FF3333]">{COMPANY_EMAIL}</strong>.
              </p>
              <div className="p-4 rounded-xl bg-surface-subtle border border-surface-border max-w-md mx-auto text-left rtl:text-right text-xs font-mono">
                <div className="text-[#DD2A40] dark:text-[#FF3333] font-bold mb-1">{selectedRes?.title}</div>
                <div className="text-[#667085]">Tier: {orderDetails.licenseTier}</div>
                <div className="text-[#667085]">Recipient: {orderDetails.email}</div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 rounded-xl bg-surface-subtle border border-surface-border text-xs text-black dark:text-white dark:text-white outline-none focus:border-[#DD2A40]"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3.5 rounded-xl bg-gradient-to-r from-[#DD2A40] to-[#DD2A40] dark:from-[#DD2A40] dark:to-[#FF3333] text-white dark:text-[#000000] font-bold text-xs shadow-md flex items-center justify-center gap-2 disabled:opacity-50 transition-all cursor-pointer"
              >
                {isSubmitting ? (
                  <>
