...
export default function GovernanceLattice({ className = '', interactive = true }) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // New Palette Colors: #E31F1F, #FF3333, #E4E4E4
      const nodeColorPrimary = isDark ? '#FF3333' : '#E31F1F';
      const nodeColorAnchor = isDark ? '#E4E4E4' : '#E31F1F';
      const lineColor = isDark ? 'rgba(255, 51, 51,' : 'rgba(227, 31, 31,';
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        ctx.fillStyle = n.isAnchor ? nodeColorAnchor : (isDark ? 'rgba(254, 205, 211, ' + alpha + ')' : 'rgba(227, 31, 31, ' + alpha + ')');
        ctx.fill();

        if (n.isAnchor) {
          ctx.beginPath();
          ctx.arc(n.x, n.y, n.radius * 3.5 + pulse * 2, 0, Math.PI * 2);
          ctx.strokeStyle = isDark ? 'rgba(255, 51, 51, 0.4)' : 'rgba(227, 31, 31, 0.3)';
          ctx.lineWidth = 1.2;
          ctx.stroke();
        }
      }
      ...
      if (mouse.x > 0 && mouse.y > 0) {
        for (let i = 0; i < nodes.length; i++) {
          ...
          if (dist < mouse.radius) {
            ...
            ctx.lineTo(n.x, n.y);
            ctx.strokeStyle = isDark ? 'rgba(254, 205, 211, ' + alpha + ')' : 'rgba(227, 31, 31, ' + alpha + ')';
            ctx.lineWidth = 1.5;
            ctx.stroke();
          }
        }
